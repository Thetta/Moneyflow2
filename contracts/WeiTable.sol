...
contract WeiTable is IWeiReceiver, Ownable {
	...
	event FundStart(uint _eId, uint _neededAmount);
	...
	function isNeedsMoneyExpenseAt(uint _eId) internal view returns(bool) {
		...
		}else if(expenses[_eId].isFund) {
			return (expenses[_eId].totalWeiReceived < getDebtMultiplierAt(_eId)*expenses[_eId].neededAmount);
		}else {
	...
	function processFundsSplitterAt(uint _eId, uint _currentFlow, uint _amount) internal {
		require(_amount>=getMinWeiNeededAt(_eId));
	...
	function getTotalWeiNeededFundAt(uint _eId, uint _inputWei)public view returns(uint) {
		uint need;
		emit FundStart(_eId, expenses[_eId].neededAmount);
		if(expenses[_eId].neededAmount==0) {
			emit FundNeed(_eId, _inputWei, _inputWei);
			return _inputWei;
		}

		if(getDebtMultiplierAt(_eId)*expenses[_eId].neededAmount > expenses[_eId].totalWeiReceived) {
			need = getDebtMultiplierAt(_eId)*expenses[_eId].neededAmount - expenses[_eId].totalWeiReceived;
		}else {
			need = 0;
		}

		emit FundNeed(_eId, _inputWei, need);
	...
	function addAbsoluteExpense(uint _neededAmount, bool _isPeriodic, bool _isAccumulateDebt, uint _periodHours)public onlyOwner {
		expenses[nodesCount] = Expense(
			_neededAmount, 0,
			_periodHours, _isPeriodic, _isAccumulateDebt,
	...
	function addFund(uint _neededAmount, bool _isPeriodic, bool _isAccumulateDebt, uint _periodHours)public onlyOwner {
		expenses[nodesCount] = Expense(
			_neededAmount, 0,
			_periodHours, _isPeriodic, _isAccumulateDebt,
	...
	function addRelativeExpense(uint _neededPpm, bool _isPeriodic, bool _isAccumulateDebt, uint _periodHours)public onlyOwner {
		expenses[nodesCount] = Expense(
			0, _neededPpm,
			_periodHours, _isPeriodic, _isAccumulateDebt,
			0, false, true, 0, 0, false
