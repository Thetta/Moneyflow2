...
contract('WeiExpense', (accounts) => {
	...
	it('Should revert when trying to add rel to abs splitter', async () => {
		var abs = await WeiAbsoluteExpense.new(1e5, 1e5);
	...
	it('Should process with WeiAbsoluteExpenseWithPeriod, then 25 hours, then needs again', async () => {
		...
		var struct = {};
		var balance0 = new web3.utils.BN(await web3.eth.getBalance(creator));
		...
		var balance = new web3.utils.BN(await web3.eth.getBalance(creator));
		assert.equal(balance.sub(balance0), 1000*multiplier, 'Should get amount');
		...
		var balance2 = new web3.utils.BN(await web3.eth.getBalance(creator));
		assert.equal(balance2.sub(balance0).toNumber(), 2000*multiplier, 'Should get amount');
	...
	it('Should process with WeiAbsoluteExpenseWithPeriod, then 75 hours, then needs again x3', async () => {
		...
		assert.equal(balance.sub(balance0).toNumber(), 1000*multiplier, 'Should get amount');
		...
		var balance2 = new web3.utils.BN(await web3.eth.getBalance(creator));
		assert.equal(balance2.sub(balance0).toNumber(), 4000*multiplier, 'Should get amount');
	...
	it('Should process with WeiSplitter + 3 WeiAbsoluteExpense', async () => {
		...
		assert.equal(weiAbsoluteExpense1Balance, 1*multiplier, 'resource point received from splitter');

		var weiAbsoluteExpense2Balance = await web3.eth.getBalance(weiAbsoluteExpense2.address);
		assert.equal(weiAbsoluteExpense2Balance, 2*multiplier, 'resource point received from splitter');

		var weiAbsoluteExpense3Balance = await web3.eth.getBalance(weiAbsoluteExpense3.address);
		assert.equal(weiAbsoluteExpense3Balance, 3*multiplier, 'resource point received from splitter');
	});
	...
	it('Should process with WeiSplitter + 3 WeiRelativeExpenseWithPeriod', async () => {
		...
		assert.equal((await web3.eth.getBalance(rel1.address)), 100*multiplier);
		assert.equal((await web3.eth.getBalance(rel2.address)), 250*multiplier);
		assert.equal((await web3.eth.getBalance(rel3.address)), 370*multiplier);
		...
		// should end up in the outputs
		assert.equal((await web3.eth.getBalance(rel1.address)), 200*multiplier);
		assert.equal((await web3.eth.getBalance(rel2.address)), 500*multiplier);
		assert.equal((await web3.eth.getBalance(rel3.address)), 740*multiplier);
